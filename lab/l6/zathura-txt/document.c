...
static int compare_pages(const txt_document_page_meta_t* page1, const txt_document_page_meta_t* page2);
static bool read_archive(txt_document_t* txt_document, const char* archive, girara_list_t* supported_extensions);
static char* get_extension(const char* path);
static void txt_document_page_meta_free(txt_document_page_meta_t* meta);
...
txt_document_open(zathura_document_t* document) {
    ...
    txt_document_t *txt_document = g_malloc0(sizeof(txt_document));
    ...
    txt_document->pages = girara_sorted_list_new2((girara_compare_function_t)
                                                          compare_pages,
                                                  (girara_free_function_t) txt_document_page_meta_free);
    if (txt_document->pages == NULL) {
        goto error_free;
    }

    /* read files recursively */
    if (read_archive(txt_document, path, supported_extensions) == false) {
        goto error_free;
    }
    ...
    zathura_document_set_number_of_pages(document, girara_list_size(txt_document->pages));
...
txt_document_free(zathura_document_t* UNUSED(document), void* data) {
    ...
    /* remove page list */
    if (txt_document->pages != NULL) {
        girara_list_free(txt_document->pages);
    }
...
static void
txt_document_page_meta_free(txt_document_page_meta_t* meta) {
    if (meta == NULL) {
        return;
    }
...
get_pixbuf_size(GdkPixbufLoader* loader, int width, int height, gpointer data) {
    txt_document_page_meta_t *meta = (txt_document_page_meta_t *) data;
...
static bool
read_archive(txt_document_t* txt_document, const char* archive, girara_list_t* supported_extensions) {
    ...
    while ((r = archive_read_next_header(a, &entry)) != ARCHIVE_EOF) {
        ...
        GIRARA_LIST_FOREACH(supported_extensions,
        char*, iter, ext)
        if (g_strcmp0(extension, ext) == 0) {
            txt_document_page_meta_t *meta = g_malloc0(sizeof(txt_document_page_meta_t));
            ...
            if (meta->width > 0 && meta->height > 0) {
                girara_list_append(txt_document->pages, meta);
            } else {
                txt_document_page_meta_free(meta);
            }

            break;
        }
        GIRARA_LIST_FOREACH_END(supported_extensions,
        char*, iter, ext);
...
static int
compare_pages(const txt_document_page_meta_t* page1, const txt_document_page_meta_t* page2) {
