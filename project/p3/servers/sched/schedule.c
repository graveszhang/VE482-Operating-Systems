...
#define largest_deadline 100

#define default_sched 0
#define lottery_sched 1
#define edf_sched 2
static int scheduler = default_sched;
...
int lottery(void) {
    ...
    for (i = 0, rmp = schedproc; i < NR_PROCS; ++i, ++rmp) {
        if ((rmp->flags & IN_USE)) {
            rmp->lottery_num = MIN_USER_Q - rmp->priority;
    ...
    printf("Lottery ticket: %d\n", ticket);
    base = 0;
    for (i = 0, rmp = schedproc; i < NR_PROCS; ++i, ++rmp) {
        if ((rmp->flags & IN_USE)) {
            if ((base += rmp->lottery_num) >= ticket) {
                printf("Process: %d\n", getpid());
