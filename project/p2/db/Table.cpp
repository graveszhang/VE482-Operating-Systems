...
bool Table::hasCopy(KeyType key){
    return (this->keyMap.find(key + "_copy") != this->keyMap.end());
}

bool Table::Duplicate(Iterator &its) {
    auto keyCopy = its->key() + "_copy";
    if (keyMap.find(keyCopy) != keyMap.end()) {
        return false;
    } else{
        tmp.emplace_back(keyCopy, its.it->datum);
        return true;
    }
//    for (unsigned i = 0; i < its.size(); i++){
//       insertByIndex(its[i]->key()+"_copy", std::move(its[i].it->datum));
//    }
}

void Table::Duplicate_helper() {
    for (auto &it : tmp) {
...
void Table::deleteRow(const Table::Object object) {
    this->data.erase(object.it);
    this->keyMap.erase(object.key());
}
...
Table::Iterator Table::getValueByIndex(KeyType key) {
    if (keyMap.find(key) == keyMap.end()) {
        return this->end();
    }
    return this->begin() + keyMap.find(key)->second;
}
