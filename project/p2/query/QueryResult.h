...
class DisplayResult : public SuceededQueryResult {
    const std::string result;
public:

    bool display() override { return true; }
    explicit DisplayResult(std::string op) : result(std::move(op)) {}
...
class ListenMsgResult : public SuceededQueryResult {
    std::string msg;
    std::string filename;
public:
    bool display() override { return true; }

    std::string info ()  override { return filename; }

    ListenMsgResult(const char *qname, const std::string &msg) {
        std::stringstream ss;
